    ...
    public class Statistiche_DAL : Base_DAL
    {
                                ...
                                if (dt != null && dt.Rows != null && dt.Rows.Count > 0)
                                {
                                    foreach (DataRow riga in dt.Rows)
                                    {
                                        try
                                        {
                                            ...
                                            statisticheRicavi.Costo = riga.Field<decimal?>("costo");
                                            statisticheRicavi.DocumentoAllegato = (fatturato != null && (bool)fatturato) ? riga.Field<string>("docFattura") : riga.Field<string>("docOfferta");
                                            statisticheRicavi.Pregresso = riga.Field<bool?>("pregresso") ?? false;
        ...
        public List<StatisticheCosti> GetStatisticheCosti(string filtriLavorazione, ref Esito esito) 
        {
            ...
            try
            {
                using (SqlConnection con = new SqlConnection(sqlConstr))
                {
                    string query = CreaQuery_SenzaFiltriCosti(filtriLavorazione);
        ...
        public List<StatisticheCosti> GetStatisticheCosti(string filtriLavorazione, string filtriCosti, ref Esito esito) 
        {
            ...
            try
            {
                using (SqlConnection con = new SqlConnection(sqlConstr))
                {
                    string query = CreaQuery_Costi(filtriLavorazione, filtriCosti);
        ...
        public List<StatisticheCosti> GetStatisticheCostiListino(string elencoLavorazioni, ref Esito esito) 
        {
            ...
            try
            {
                using (SqlConnection con = new SqlConnection(sqlConstr))
                {
                    string filtriLavorazione = "  AND a.codice_lavoro in (" + elencoLavorazioni + ") ";
                    string query = CreaQuery_Listino(filtriLavorazione);
        ...
        #region QUERY COSTI
        private string CreaQuery_SenzaFiltriCosti(string filtriLavorazione)
        {
            ...
            //        " group by a.id_cliente, b.ragioneSociale, a.produzione,a.codice_lavoro, a.lavorazione, c.ordine, e.protocollo_riferimento, e.pathDocumento, h.pathDocumento, h.pregresso, k.nome, forn.ragioneSociale, coll.cognome, coll.nome, d.id, d.costo, d.data, coll.nomeSocieta " +

            query += " UNION " + CreaQuery_CollaboratoriAssunti(filtriLavorazione, "");
            ...
            //"select  3 progressivo, d.id, a.id_cliente, b.ragioneSociale cliente, e.protocollo_riferimento 'numeroFattura', c.ordine, a.codice_lavoro, d.data data, a.lavorazione, a.produzione, 'Collaboratori' gruppo, k.nome sottogruppo, COALESCE (coll.cognome+' '+coll.nome + COALESCE(' (' + NULLIF(LTRIM(RTRIM(coll.nomeSocieta)), '') + ')', ''), forn.ragioneSociale) fornitore, 0.00 listino, d.costo costo, e.pathDocumento as documento, h.pregresso, '0' as codiceLavoro2   " +
            ...
            query += " UNION " + CreaQuery_CollaboratoriAFattura(filtriLavorazione, "");
            #endregion
            ...
            //                        "select  4 progressivo, d.id, a.id_cliente, b.ragioneSociale cliente, e.protocollo_riferimento 'numeroFattura', c.ordine, a.codice_lavoro, d.data data, a.lavorazione, a.produzione, l.nome gruppo, k.nome sottogruppo, COALESCE (coll.cognome+' '+coll.nome + COALESCE(' (' + NULLIF(LTRIM(RTRIM(coll.nomeSocieta)), '') + ')', ''), forn.ragioneSociale) fornitore, 0.00 listino, d.costo costo, e.pathDocumento  as documento, h.pregresso, '0' as codiceLavoro2   " +
            ...
            //                        " group by a.id_cliente, b.ragioneSociale, a.produzione,a.codice_lavoro, a.lavorazione, c.ordine, e.protocollo_riferimento, e.pathDocumento, h.pathDocumento, h.pregresso, k.nome, l.nome, forn.ragioneSociale, coll.cognome, coll.nome, d.id, d.costo, d.data, coll.nomeSocieta " +

            query += " UNION " + CreaQuery_Diaria(filtriLavorazione, "");
            #endregion
            ...
            //" group by a.id_cliente, b.ragioneSociale, a.produzione,a.codice_lavoro, a.lavorazione, c.ordine, e.protocollo_riferimento, e.pathDocumento, h.pathDocumento, h.pregresso, k.nome, l.nome, forn.ragioneSociale, coll.cognome, coll.nome, d.costo,coll.nomeSocieta " +

            query += " UNION " + CreaQuery_ElementiRestanti(filtriLavorazione, "");
            #endregion
            query += " order by cliente, a.codice_lavoro, progressivo";

            return query;
        }

        private string CreaQuery_Costi(string filtriLavorazione, string filtriCosti)
        {
            // SOLO I COSTI

            #region COLLABORATORI ASSUNTI
            //string query =                       //"select distinct 2 progressivo, d.id, a.id_cliente, b.ragioneSociale cliente, e.protocollo_riferimento 'numeroFattura', c.ordine, a.codice_lavoro, d.data data, a.lavorazione, a.produzione, 'Collaboratori' gruppo, k.nome sottogruppo, COALESCE (coll.cognome+' '+coll.nome , forn.ragioneSociale) fornitore, 0.00 listino, d.costo costo, f.descrizione contratto, e.pathDocumento as documento, h.pregresso  " +
            ...
            string query = CreaQuery_CollaboratoriAssunti(filtriLavorazione, filtriCosti);
            ...
            query += " UNION " + CreaQuery_CollaboratoriAFattura(filtriLavorazione, filtriCosti);
            ...
            query += " UNION " + CreaQuery_Diaria(filtriLavorazione, filtriCosti);
            #endregion
            ...
            //            " group by a.id_cliente, b.ragioneSociale, a.produzione,a.codice_lavoro, a.lavorazione, c.ordine, e.protocollo_riferimento, e.pathDocumento, h.pathDocumento, h.pregresso, l.nome, k.nome, forn.ragioneSociale, coll.cognome, coll.nome, d.costo, coll.nomeSocieta " +

            query += " UNION " + CreaQuery_ElementiRestanti(filtriLavorazione, filtriCosti);
            #endregion
            query += " order by cliente, a.codice_lavoro, progressivo";
        ...
        //private string CreaQueryListino_OLD(string elencoLavorazioni)
        //{
        ...
        #region QUERY DELLE SEZIONI
        private string CreaQuery_Listino(string filtriLavorazione)
        {
            // SOLO IL PREZZO DI LISTINO
            string query = "select  1 progressivo, 0, a.id_cliente, b.ragioneSociale cliente, e.protocollo_riferimento 'numeroFattura', c.ordine, a.codice_lavoro, min(a.data_inizio_lavorazione) data, a.lavorazione, a.produzione, '' gruppo, '' sottogruppo, '' fornitore, d.prezzo listino, 0.00  costo, MIN(COALESCE(e.pathDocumento,h.pathDocumento)) AS documento, h.pregresso, a.codice_lavoro as codiceLavoro2  " +
                                    "from tab_dati_agenda a  " +
        ...
        private string CreaQuery_CollaboratoriAssunti(string filtriLavorazione, string filtriCosti)
        {
            string query = "select distinct 2 progressivo, d.id, a.id_cliente, b.ragioneSociale cliente, e.protocollo_riferimento 'numeroFattura', c.ordine, a.codice_lavoro, d.data data, a.lavorazione, a.produzione, 'Collaboratori' gruppo, k.nome sottogruppo, COALESCE (coll.cognome+' '+coll.nome, forn.ragioneSociale) fornitore, 0.00 listino, d.costo costo, e.pathDocumento as documento, h.pregresso, '0' as codiceLavoro2   " +
                            "from tab_dati_agenda a  " +
                            "left join anag_clienti_fornitori b on b.id = a.id_cliente  " +
        ...
        private string CreaQuery_CollaboratoriAFattura(string filtriLavorazione, string filtriCosti)
        {
            string query = "select  3 progressivo, d.id, a.id_cliente, b.ragioneSociale cliente, e.protocollo_riferimento 'numeroFattura', c.ordine, a.codice_lavoro, d.data data, a.lavorazione, a.produzione, 'Collaboratori' gruppo, k.nome sottogruppo, COALESCE (coll.cognome+' '+coll.nome + COALESCE(' (' + NULLIF(LTRIM(RTRIM(coll.nomeSocieta)), '') + ')', ''), forn.ragioneSociale) fornitore, 0.00 listino, d.costo costo, e.pathDocumento as documento, h.pregresso, '0' as codiceLavoro2   " +
                            "from tab_dati_agenda a  " +
                            "left join anag_clienti_fornitori b on b.id = a.id_cliente  " +
                            "left join dati_lavorazione c on c.idDatiAgenda = a.id  " +
                            "LEFT JOIN (SELECT idDatiLavorazione, fp_netto AS costo, idFornitori, idTipoSottogruppo, idCollaboratori, id, idTipoGruppo, idTipoGenere, data FROM dati_articoli_lavorazione) d ON d.idDatiLavorazione = c.id " +
        ...
        private string CreaQuery_Diaria(string filtriLavorazione, string filtriCosti)
        {
            string query = "select  4 progressivo, d.id, a.id_cliente, b.ragioneSociale cliente, e.protocollo_riferimento 'numeroFattura', c.ordine, a.codice_lavoro, d.data data, a.lavorazione, a.produzione, l.nome gruppo, k.nome sottogruppo, COALESCE (coll.cognome+' '+coll.nome + COALESCE(' (' + NULLIF(LTRIM(RTRIM(coll.nomeSocieta)), '') + ')', ''), forn.ragioneSociale) fornitore, 0.00 listino, d.costo costo, e.pathDocumento as documento, h.pregresso, '0' as codiceLavoro2   " +
                            "from tab_dati_agenda a  " +
                            "left join anag_clienti_fornitori b on b.id = a.id_cliente  " +
                            "left join dati_lavorazione c on c.idDatiAgenda = a.id  " +
                            "LEFT JOIN (SELECT idDatiLavorazione, fp_lordo AS costo, idFornitori,idCollaboratori, idTipoGruppo, idTipoSottoGruppo, id, idTipoGenere, data  FROM dati_articoli_lavorazione ) d ON d.idDatiLavorazione = c.id  " +
        ...
        private string CreaQuery_ElementiRestanti(string filtriLavorazione, string filtriCosti)
        {
            string query = "select distinct 5 progressivo, 0, a.id_cliente, b.ragioneSociale cliente, e.protocollo_riferimento 'numeroFattura', c.ordine, a.codice_lavoro, min(a.data_inizio_lavorazione) data, a.lavorazione, a.produzione, l.nome gruppo, k.nome sottogruppo, COALESCE (coll.cognome+' '+coll.nome + COALESCE(' (' + NULLIF(LTRIM(RTRIM(coll.nomeSocieta)), '') + ')', ''), forn.ragioneSociale) fornitore, 0.00 listino, d.costo costo, e.pathDocumento as documento, h.pregresso, '0' as codiceLavoro2   " +
                            "from tab_dati_agenda a  " +
                            "left join anag_clienti_fornitori b on b.id = a.id_cliente  " +
                            "left join dati_lavorazione c on c.idDatiAgenda = a.id  " +
                            "LEFT JOIN (SELECT idDatiLavorazione, sum(fp_netto) AS costo, idFornitori,idTipoGruppo,idTipoSottogruppo, idTipoGenere, idCollaboratori, id  FROM dati_articoli_lavorazione GROUP BY idDatiLavorazione, idFornitori,idTipoGruppo,idTipoSottogruppo, idCollaboratori, idTipoGenere, id) d ON d.idDatiLavorazione = c.id " +
